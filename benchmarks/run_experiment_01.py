...
RESULTS_PATH = RESULTS_DIR / "experiment_01_results.json"
...
def extract_record_id(question):
    ...
    if not match:
        raise ValueError(
            f"Could not extract record_id: {question}"
        )

    return int(match.group(1))


def build_row_chunks(df):
    """
    Experiment 1:
    Keep each retail record as a complete semantic unit.

    This intentionally changes ONLY the chunking strategy.
    """
...
def create_vector_store(chunks, metadatas):
    ...
    return Chroma.from_texts(
        ...
        metadatas=metadatas,
        collection_name=f"retailgenai_exp01_{int(time.time() * 1000000)}"
    )
...
def main():
    ...
    print("=" * 70)
    print("RetailGenAI - EXPERIMENT 01")
    print("Row-Aware Chunking")
    print("=" * 70)
    ...
    print("\nCreating row-aware chunks...")

    start = time.perf_counter()

    chunks, metadatas = build_row_chunks(df)
    ...
    output = {
        "experiment": {
            "name": "Experiment 01 - Row-Aware Chunking",
