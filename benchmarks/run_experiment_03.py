...
def transform_query(question):
    ...
    for region in regions:
        if re.search(rf"\b{region}\b", question, re.IGNORECASE):
...
def build_vector_store(documents):
    ...
    return Chroma.from_texts(
        ...
        metadatas=metadatas,
        collection_name="experiment_03_query_transformation"
    )
...
def main():
    ...
    print("=" * 70)
    print("EXPERIMENT 3 — QUERY TRANSFORMATION")
    print("=" * 70)
    ...
    for i, item in enumerate(questions, start=1):
        ...
        if record_id is not None and record_id in retrieved_ids:

            retrieval_hits.append(1)

            rank = retrieved_ids.index(record_id) + 1
            ...
            print("Correct record not retrieved")
        ...
        if expected_match:
            expected_value = expected_match.group(0)
            is_correct = expected_value in answer
        else:
            is_correct = False
    ...
    results = {
        "experiment": "experiment_03_query_transformation",
